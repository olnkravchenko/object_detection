...
def draw_gaussian(heatmap, center, radius_x, radius_y):
    ...
    gaussian = gaussian2D((diameter_y, diameter_x), sigma_y=diameter_y / 6, sigma_x=diameter_x / 6)
...
class CenternetEncoder:
    ...
    def __call__(self, bboxes, labels):
        ...
        hm = np.zeros((self._out_height, self._out_width, self._n_classes), dtype=np.float32)
        ...
        for cls_id, bbox in zip(labels.data.numpy(), bboxes.data.numpy()):
            ...
            if h > 0 and w > 0:
                center = np.array([(box_s[0] + box_s[2]) / 2, (box_s[1] + box_s[3]) / 2], dtype=np.float32)
                center = np.round(center)
                center = np.clip(center, [0, 0], [self._out_width-1, self._out_height-1])
